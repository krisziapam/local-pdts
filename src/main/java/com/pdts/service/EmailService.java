...
public class EmailService {
    ...
    private void validateResendConfig() {
        if (resendApiKey == null || resendApiKey.isBlank()) {
            throw new IllegalStateException("RESEND_API_KEY is missing from the local environment variables.");
        }

        if (fromEmail == null || fromEmail.isBlank()) {
            throw new IllegalStateException("RESEND_FROM_EMAIL is missing from the local environment variables.");
        }
    }
